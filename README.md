...
- ~~Release artifact evidence~~ → Docker image built and `docker run --rm salmon-run:0.1.1` lists queues; PowerShell Gallery `SalmonRun` meta-module manifest validated and local `nupkg` built with `scripts/Publish-SalmonRunModule.ps1`; `docs/RELEASE.md` written with artifact set, versioning, checklist, and rollback steps.
