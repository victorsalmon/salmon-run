...
- **Current score:** 85%
- **Current behavior:** Creates `~/.salmon` task queue directories, copies `.env.example` to `~/.salmon/.env`, sets `SALMON_RUN_HOME`, copies `Orchestrator/Modules/*` and `Skills/Docker/Modules/*` to `~/.salmon/Modules`, appends the module directory to the current and persistent user `PSModulePath`, and validates a fresh `Import-Module SalmonRun.PondEngine`.
