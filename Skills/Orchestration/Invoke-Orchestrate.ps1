...
$InterclawDir = (Resolve-Path (Join-Path $PSScriptRoot '..\..')).Path
$LogDir = "$InterclawDir/Tasks/Logs"
$AgentDir = "$LogDir/agents"
$WorkingDir = "$InterclawDir/Tasks/Working"
...
function Start-CompleteAudit {
    param([string]$InterclawDir, [string]$LogDir)
...
function Show-PostWorkCompleteChecklist {
    ...
    $manualDir = Join-Path $InterclawDir 'Tasks/Manual'
    $failedDir = Join-Path $InterclawDir 'Tasks/Failed'
    ...
    function Move-ToArchive($source, $type) {
        $archiveBase = Join-Path $InterclawDir "Tasks/Archive/$type"
...
if ($DetachWatchdog) {
    ...
    $wdAlive = $existingWDPid -and (Get-Process -Id $existingWDPid -ErrorAction SilentlyContinue)
    ...
    $handoff = @(Get-ChildItem (Join-Path $InterclawDir "Tasks/Handoff/*.md") -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
    $workingDirs = @(Get-ChildItem (Join-Path $InterclawDir "Tasks/Working/*") -Directory -ErrorAction SilentlyContinue)
    ...
    foreach ($__sig in @("$InterclawDir/Tasks/stop", "$InterclawDir/Tasks/stop.code", "$InterclawDir/Tasks/stop.review")) {
    ...
    if (-not $wdAlive) {
        ...
        $null = Start-Process -FilePath $pwsh -ArgumentList @('-NoProfile', '-NoLogo', '-EncodedCommand', $encodedCmd) -WorkingDirectory $InterclawDir -WindowStyle Hidden -PassThru
        Write-Host "Watchdog launched in background" -ForegroundColor Cyan
        exit 0
    }
...
if ($orchErrors) {
    ...
    "$errorMsgs" | Out-File (Join-Path $InterclawDir "Tasks/Logs/.orchestrate-parse-error") -Encoding utf8
    exit 1
}
...
$orchModulePath = Join-Path $PSScriptRoot "..\Docker\Modules\SalmonRun.Orchestrate\SalmonRun.Orchestrate.psd1"
...
function Get-TaskCounts {
    $rootCoder = @(Get-ChildItem "$InterclawDir/Tasks/Code/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
    $review = @(Get-ChildItem "$InterclawDir/Tasks/Review/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
    $handoff = @(Get-ChildItem "$InterclawDir/Tasks/Handoff/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
    $failed = @(Get-ChildItem "$InterclawDir/Tasks/Failed/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
    $todo = @(Get-ChildItem "$InterclawDir/Tasks/ToDo/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
    $manual = @(Get-ChildItem "$InterclawDir/Tasks/Manual/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
    $paused = @(Get-ChildItem "$InterclawDir/Tasks/Paused/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
    $completeFiles = @(Get-ChildItem "$InterclawDir/Tasks/Complete/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
    $completeDirs = @(Get-ChildItem "$InterclawDir/Tasks/Complete" -Directory -ErrorAction SilentlyContinue).Count
    $working = @(Get-ChildItem "$InterclawDir/Tasks/Working/*/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' })
...
$script:SpawnedPidsRegistryPath = Join-Path $InterclawDir "Tasks/Logs/agents/.spawned-pids.json"
...
function Register-SpawnedPid {
    ...
    try {
        ...
        if ($AgentId -and (-not $data.byAgent.$AgentId)) { $data.byAgent | Add-Member -NotePropertyName $AgentId -NotePropertyValue $ProcessId -Force }
...
function Unregister-SpawnedPid {
    ...
    try {
        ...
        if ($AgentId) { $data.byAgent.PSObject.Properties.Remove($AgentId) }
        $data | ConvertTo-Json -Depth 3 | Set-Content $script:SpawnedPidsRegistryPath -Encoding utf8 -NoNewline
...
while ($true) {
    ...
    try {
        ...
        $counts = [PSCustomObject]@{
            RootCoder = @(Get-ChildItem "$InterclawDir/Tasks/Code/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
            Review = @(Get-ChildItem "$InterclawDir/Tasks/Review/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
            Working = @(Get-ChildItem "$InterclawDir/Tasks/Working/*/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
            Failed = @(Get-ChildItem "$InterclawDir/Tasks/Failed/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
            ToDo = @(Get-ChildItem "$InterclawDir/Tasks/ToDo/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
            Manual = @(Get-ChildItem "$InterclawDir/Tasks/Manual/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
            Paused = @(Get-ChildItem "$InterclawDir/Tasks/Paused/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
    ...
    if ($orchAlive) {
        $script:lastOrchPid = $orchPid
        Get-ChildItem "$InterclawDir/Tasks/Logs/.orchestrate-reload-count-*" -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue
    }
    ...
    if ($FinishWithAudit -and $auditProcess) {
        if ($auditProcess.HasExited -and -not $auditExited) {
            $auditExited = $true
            $auditCodeCount = @(Get-ChildItem "$InterclawDir/Tasks/Code/*.md" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne '.gitkeep' }).Count
    ...
    if ($allEmpty -and -not $orchAlive) {
        if ($FinishWithAudit -and -not $auditLaunched) {
            $auditProcess = Start-CompleteAudit -InterclawDir $InterclawDir -LogDir $LogDir
            if ($auditProcess) {
                $auditLaunched = $true
        ...
        if ($manualRemaining -gt 0 -or $failedRemaining -gt 0) {
            ...
            $checklistResult = Show-PostWorkCompleteChecklist -InterclawDir $InterclawDir
    ...
    if (-not $orchAlive -and -not $allEmpty) {
        $orchActivePathMutex = Join-Path $InterclawDir "Tasks/Logs/.orchestrator-active"
    ...
    if (-not $orchAlive -and -not $allEmpty) {
        $continueFile = Join-Path $InterclawDir "Tasks/Logs/.orchestrator-continue"
        ...
        $initErrPath = Join-Path $InterclawDir "Tasks/Logs/.orchestrator-init-error"
        try {
        ...
        if ($isPostAuditRelaunch) {
            ...
            $reloadCountPath = Join-Path $InterclawDir "Tasks/Logs/.orchestrate-reload-count-$PID"
            ...
            if ($reloadCount -ge 10) {
                ...
                Set-Content -Path (Join-Path $InterclawDir "Tasks/stop") -Value "stop" -Encoding utf8 -NoNewline
                ...
                Set-Content -Path (Join-Path $InterclawDir "Tasks/Logs/.orchestrate-crash-report") -Value $report -Encoding utf8 -NoNewline
                exit 1
            }
        ...
        foreach ($f in $orphanFiles) {
            ...
            $destDir = "$InterclawDir/Tasks/$destBase"
        ...
        . (Join-Path $InterclawDir "Skills/DevOps/Git/Invoke-WorktreeSetup.ps1")
        . (Join-Path $InterclawDir "Skills/DevOps/Git/Invoke-WorktreeCleanup.ps1")
        ...
        if ($budget) {
            ...
            foreach ($entry in $exceeded) {
                ...
                foreach ($dir in @("Code", "Review")) {
                    $p = Join-Path $InterclawDir "Tasks/$dir/$fileName"
    ...
    if ($previousQueueCounts) {
        ...
        if ($noProgress -and -not $allEmpty) {
    ...
    $stopSignalPaths = @(
        ...
        (Join-Path $InterclawDir "Tasks/stop.review"),
        (Join-Path $InterclawDir "Tasks/stop"),
        (Join-Path $InterclawDir "Tasks/Logs/.orchestrator-stop")
    )
