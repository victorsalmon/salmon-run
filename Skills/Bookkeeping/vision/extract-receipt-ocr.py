...
def main():
    ...
    if not api_key and args.api_key_env != "OPENROUTER_API_KEY":
        api_key = os.environ.get("OPENROUTER_API_KEY")
    if not api_key:
        print(f"ERROR: {args.api_key_env} (or OPENROUTER_API_KEY) environment variable not set", file=sys.stderr)
        sys.exit(1)
