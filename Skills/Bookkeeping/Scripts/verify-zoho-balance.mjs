...
const { parseArgs, loadEntityConfig, sleep, parseDate } = require('./lib/zoho-common.js');
...
async function fetchAllBankTxns(auth, acctId) {
  ...
  while (hasMore && page <= 50) {
    const url = `https://www.zohoapis.com/books/v3/banktransactions?account_id=${acctId}&organization_id=${ORG_ID}&per_page=200&page=${page}&sort_column=date&sort_order=A`;
