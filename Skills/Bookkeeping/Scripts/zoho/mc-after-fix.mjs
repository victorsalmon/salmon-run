...
async function main() {
  ...
  const stmts = [
    { date: "2025-04-09", bal: 1282.78, label: "Apr 9, 2025" },
  ...
  console.log("MC 6258 — AFTER $21.78 removal:\n");
  ...
  for (const s of stmts) {
    ...
    const marker = gap !== null && Math.abs(gap) > 1 ? " ◄ OUTLIER" : "";
    const delta = (prevGap !== null && gap !== null) ? (gap - prevGap) : null;
    const deltaStr = delta !== null ? (delta > 0 ? `+${delta.toFixed(2)}` : delta.toFixed(2)) : "";
    console.log(`${s.date} | $${s.bal.toString().padStart(7)} | $${zStr.padStart(7)} | ${gapStr.padStart(6)}${marker} | ${s.label}  [Δ ${deltaStr}]`);
