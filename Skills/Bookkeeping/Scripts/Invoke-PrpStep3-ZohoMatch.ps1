<#
.SYNOPSIS
    PRP Step 3: Match Zoho transactions against sidecar data per period.
.DESCRIPTION
...
.EXAMPLE
    Invoke-PrpStep3-ZohoMatch.ps1 -ZohoAll $zohoAll -SidecarData $sidecars -SidecarPeriods $periods
#>
...
Write-Progress -Activity "[PRP Step 3]" -Status $stepName
...
if ($WhatIfPreference) {
    Write-Information "[PRP STEP 3] WhatIf: would compare $($ZohoAll.Count) Zoho transactions across $($SidecarPeriods.Count) periods" -Tags PRP
    return [PSCustomObject]@{
        StepNumber    = $stepNumber
        Passed        = $true
...
foreach ($period in $SidecarPeriods) {
    ...
    Write-Information "[PRP STEP 3] Period $periodLabel`: $sidecarCount sidecar, $zohoCount Zoho, diff=$diff [$status]" -Tags PRP
}
...
Write-Progress -Activity "[PRP Step 3]" -Status $stepName -Completed
...
return [PSCustomObject]@{
    ...
    NextSteps     = @(
        $(if ($allPassed) { "Proceed to Step 4: Discover PDF Statements" }
          elseif ($IsPlaidImmutable) { "Record discrepancies in remediation report — Plaid-immutable, process in Zoho UI" }
          else { "Fix discrepancies via API (DELETE extras / POST missing), then re-run Step 3" })
    )
}
