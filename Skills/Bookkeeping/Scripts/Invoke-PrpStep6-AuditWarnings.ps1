<#
.SYNOPSIS
    PRP Step 6: Scan for audit warning patterns.
.DESCRIPTION
...
.EXAMPLE
    Invoke-PrpStep6-AuditWarnings.ps1 -ZohoAll $zohoAll -EntityName "intersite-consulting"
#>
...
if ($WhatIfPreference) {
    Write-Information "[PRP STEP 6] WhatIf: would scan $($ZohoAll.Count) transactions for 10 warning patterns" -Tags PRP
...
if ($passed) {
    Write-Information "[PRP STEP 6] PASSED — No warning patterns detected" -Tags PRP
} else {
    Write-Warning "[PRP STEP 6] FAILED — $($warnings.Count) warning pattern(s)"
...
return [PSCustomObject]@{
    ...
    NextSteps        = @(
        $(if ($passed) { "Proceed to Step 7: Balance Forward Verification" }
          elseif ($IsPlaidImmutable) { "Append warnings to remediation report for Zoho UI processing" }
          else { "Execute fix strategies via API using cached token, re-fetch, re-run Step 6" })
    )
}
