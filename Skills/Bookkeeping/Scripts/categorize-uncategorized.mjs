...
const CATEGORY_ACCT_IDS = {
  'Automobile Expense':           '93310000000000424',
...
const VENDOR_RULES = [
  ...
  { rx: /PETRO|SHELL|CHEVRON|CHV\d+|GAS\b|CO-OP|CANCO|SUPER\s*SAVE|ESSO|MOBIL(?!E)/i, cat: 'Automobile Expense' },
  { rx: /KAL[-?\s]*TIRE|LORDCO|IMPARK|PARKING/i,                               cat: 'Automobile Expense' },
  { rx: /ICBC/i,                                                               cat: 'Automobile Expense' },
...
async function main() {
  ...
  try {
    if (process.env.ZOHO_BOOKS_ID && process.env.ZOHO_BOOKS_SECRET && process.env.ZOHO_BOOKS_REFRESH) {
      ...
      // Use the new fullaccess token directly
      creds = {
        clientId: 'REDACTED_ZOHO_TOKEN',
        clientSecret: 'REDACTED_ZOHO_CLIENT_SECRET',
        refreshToken: 'REDACTED_ZOHO_TOKEN',
        orgId: ORG_ID,
      };
    }
  } catch (e) {
