<#
.SYNOPSIS
    PRP Step 8: Hybrid reconciliation with verification.
.DESCRIPTION
...
.EXAMPLE
    Invoke-PrpStep8-Reconcile.ps1 -AccountId "12345" -PeriodEnd "2026-01-31" -ClosingBalance 1500.00 -Token "..." -OrgId "925048093"
#>
...
Write-Progress -Activity "[PRP Step 8]" -Status $stepName
...
if ($WhatIfPreference) {
    Write-Information "[PRP STEP 8] WhatIf: would attempt API reconciliation for AccountId=$AccountId PeriodEnd=$PeriodEnd ClosingBalance=$ClosingBalance" -Tags PRP
    Write-Information "[PRP STEP 8] WhatIf: Account is $(if ($IsPlaidImmutable) { 'Plaid-immutable — would skip directly to browserless fallback' } else { 'API-mutable — would try POST /bankaccounts/{id}/reconciliation' })" -Tags PRP
    return [PSCustomObject]@{
        StepNumber   = $stepNumber
        Passed       = $true
...
# Step 8a: Attempt API Reconciliation
if (-not $IsPlaidImmutable) {
    Write-Information "[PRP STEP 8] Step 6a: Attempting API reconciliation for period ending $PeriodEnd" -Tags PRP
    ...
    Write-Information "[PRP STEP 8] Plaid-immutable account — skipping API reconciliation attempt" -Tags PRP
    $method = "plaid_skip"
}

# Step 8b: Verify via Confirm-ReconciliationStatus.ps1
...
if ($passed) {
    Write-Information "[PRP STEP 8] PASSED — $detail" -Tags PRP
} else {
    Write-Warning "[PRP STEP 8] FAILED — $detail"
}

# Step 8c: Browserless fallback
$browserlessResult = $null
if (-not $passed -and $IsPlaidImmutable) {
    Write-Information "[PRP STEP 8] Step 8c: Attempting browserless reconciliation..." -Tags PRP
...
if (-not $passed -and -not $browserlessResult) {
    Write-Information "[PRP STEP 8] Step 8c: Browserless fallback instructions" -Tags PRP
...
Write-Progress -Activity "[PRP Step 8]" -Status $stepName -Completed
...
return [PSCustomObject]@{
    ...
    NextSteps           = @(
        $(if ($passed) { "Proceed to Step 9: Categorization Audit (Post-Recon)" }
          else { "Run Invoke-PrpBrowserlessReconcile.ps1 manually, then re-run Step 8" })
    )
}
