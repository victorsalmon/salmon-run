<#
.SYNOPSIS
    PRP Step 10: Post-recon Manual Category Reasonableness Checks.
.DESCRIPTION
...
.EXAMPLE
    Invoke-PrpStep10-CategoryChecks.ps1 -ZohoAll $zohoAll -AllExpenses $allExpenses
#>
...
if ($WhatIfPreference) {
    Write-Information "[PRP STEP 10] WhatIf: would run 5 rubric checks on categorized dataset" -Tags PRP
...
if ($passed) {
    Write-Information "[PRP STEP 10] PASSED — All 5 rubric checks pass" -Tags PRP
} else {
    Write-Warning "[PRP STEP 10] FAILED — $($flaggedIssues.Count) issue(s) found"
...
return [PSCustomObject]@{
    ...
    NextSteps     = @(
        $(if ($passed) { "Proceed to Step 11: Update Status" }
          elseif ($IsPlaidImmutable) { "Add flagged issues to remediation report for Zoho UI processing" }
          else { "Fix flagged categories via PUT /expenses/{id} or journal entry, re-run Step 10" })
    )
}
