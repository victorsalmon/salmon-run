...
# --- Pipeline Warnings ---
$pipelineWarningsPath = "$booksRoot\.pipeline-warnings.json"
if (Test-Path $pipelineWarningsPath) {
    try {
        $pipelineWarnings = Get-Content $pipelineWarningsPath -Raw -Encoding UTF8 | ConvertFrom-Json
        if ($pipelineWarnings.Count -gt 0) {
            Write-Status "`n══════════════════════════════════════════" "Cyan"
            Write-Status "  Accumulated Pipeline Warnings" "Cyan"
            Write-Status "══════════════════════════════════════════" "Cyan"
            $errorsCount = @($pipelineWarnings | Where-Object { $_.severity -eq 'error' }).Count
            $warningsCount = @($pipelineWarnings | Where-Object { $_.severity -eq 'warning' }).Count
            Write-Status "  Total: $($pipelineWarnings.Count) ($errorsCount errors, $warningsCount warnings)" $(if ($errorsCount -gt 0) {'Red'} elseif ($warningsCount -gt 0) {'Yellow'} else {'Green'})
            foreach ($pw in $pipelineWarnings) {
                $icon = switch ($pw.severity) {
                    'error' { '❌' }
                    'warning' { '⚠' }
                    'info' { 'ℹ' }
                    default { '·' }
                }
                Write-Status "  $icon [$($pw.stage)] $($pw.message)" $(if ($pw.severity -eq 'error') {'Red'} elseif ($pw.severity -eq 'warning') {'Yellow'} else {'Gray'})
            }
        }
    } catch {
        Write-Status "  Could not parse pipeline warnings: $_" "Red"
    }
}
