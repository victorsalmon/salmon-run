# Used by: Skills/Bookkeeping/books/reconciliation/_pre-recon-pipeline.md (Step 6c fallback)
# Invoked by: Invoke-PrpStep6-Reconcile.ps1
    ...
    if ($inTargetSection) {
        if ($line -match '^\|(.+)\|(.+)\|$') {
            ...
            if ($periodStr -match '–\s*([A-Za-z]+ \d+,? \d{4})') {
                ...
                $endDateStr = $endDateStr.Trim() -replace ',', ''
                [datetime]$endDate = $null
                $fmts = @("MMM dd yyyy", "MMMM dd yyyy", "MMM d yyyy", "MMMM d yyyy")
                foreach ($fmt in $fmts) {
                    if ([datetime]::TryParseExact($endDateStr, $fmt, [System.Globalization.CultureInfo]::CurrentCulture, [System.Globalization.DateTimeStyles]::None, [ref]$endDate)) {
                        break
                    }
                }
                if (-not $endDate) {
