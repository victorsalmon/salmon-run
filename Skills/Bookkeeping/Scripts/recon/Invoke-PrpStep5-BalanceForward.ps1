<#
.SYNOPSIS
    PRP Step 5: Three-way balance forward verification.
.DESCRIPTION
...
$openingBalanceAdjustment = 0.0
$firstPeriod = $SidecarPeriods[0]
$expectedOpeningBalance = if ($firstPeriod.opening_balance) { [double]$firstPeriod.opening_balance } else { 0.0 }

if ($Token -and $OrgId -and $AccountId) {
    Write-Information "[PRP STEP 5] Fetching account opening balance from Zoho..." -Tags PRP
    try {
        $acctUri = "https://www.zohoapis.com/books/v3/bankaccounts/$AccountId?organization_id=$OrgId"
        $acctResp = Invoke-RestMethod -Uri $acctUri -Headers $Headers
        $zohoOpeningBalance = [double]($acctResp.bankaccount.opening_balance)
        Write-Information "[PRP STEP 5] Zoho opening balance: $zohoOpeningBalance" -Tags PRP

        if ($expectedOpeningBalance -ne 0) {
            $diff = [math]::Round(($zohoOpeningBalance - $expectedOpeningBalance), 2)
            if ([math]::Abs($diff) -gt 0.01) {
                $openingBalanceAdjustment = $diff
                Write-Warning "[OPENING BALANCE] Account opening balance in Zoho ($zohoOpeningBalance) differs from expected ($expectedOpeningBalance) by $diff. Adjusting all period comparisons by this offset."
            } else {
                Write-Information "[PRP STEP 5] Opening balance matches expected ($zohoOpeningBalance vs $expectedOpeningBalance)" -Tags PRP
            }
        } else {
            Write-Information "[PRP STEP 5] First period has no opening_balance — skipping comparison" -Tags PRP
        }
    } catch {
        Write-Warning "[PRP STEP 5] Could not fetch account opening balance: $_ — proceeding without adjustment"
    }
} else {
    Write-Information "[PRP STEP 5] No token/OrgId/AccountId provided — skipping opening balance verification" -Tags PRP
}
...
foreach ($period in $SidecarPeriods) {
    ...
    # Zoho net flow (adjusted for opening balance difference if applicable)
    ...
    if ([math]::Abs($openingBalanceAdjustment) -gt 0.01) {
        $adjNote = "(opening balance adjustment: $openingBalanceAdjustment)"
        $zohoNet = [math]::Round(($zohoNet + $openingBalanceAdjustment), 2)
    } else {
        $adjNote = $null
    }
    ...
    $periodStatus[$periodLabel] = @{
        ...
        Passed         = $periodOk
        OpeningAdj     = if ($adjNote) { $openingBalanceAdjustment } else { $null }
    }
