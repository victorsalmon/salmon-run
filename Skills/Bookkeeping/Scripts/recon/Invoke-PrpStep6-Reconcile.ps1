<#
.SYNOPSIS
    PRP Step 6: Hybrid reconciliation with verification.
.DESCRIPTION
...
if (-not $IsPlaidImmutable) {
    ...
    try {
        $reconResult = Invoke-RestMethod -Uri $reconUri -Headers $Headers -Method POST -Body $reconBody -ContentType "application/json" -ErrorAction Stop
...
if (-not $passed) {
    Write-Information "[PRP STEP 6] Step 6c: API reconciliation failed ($detail) — falling back to browserless..." -Tags PRP
