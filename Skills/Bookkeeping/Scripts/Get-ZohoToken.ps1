...
function Get-ZohoToken {
    ...
    $refreshField = $null
    if ($cred -and $cred.ZOHO_BOOKS_REFRESH) {
        $refreshField = "ZOHO_BOOKS_REFRESH"
    } elseif ($cred -and $cred.ZOHO_BOOKS_REFRESH_TOKEN) {
        $refreshField = "ZOHO_BOOKS_REFRESH_TOKEN"
    } elseif ($env:ZOHO_BOOKS_REFRESH) {
        $cred = [PSCustomObject]@{
        ...
        throw "No Zoho refresh token found — checked Docker secrets bundle (ZOHO_BOOKS_REFRESH) and env vars (ZOHO_BOOKS_REFRESH)"
    }
