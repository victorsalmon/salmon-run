...
BeforeAll {
    ...
    $script:TargetPath = Join-Path $script:RepoRoot "Skills" "Docker" "Check-KeyAge.ps1"
}

Describe "Check-KeyAge.ps1" -Tag "Docker" {
    ...
    It "has param block with DaysBeforeExpiry, Profile, and Region parameters" {
        $content = Get-Content $script:TargetPath -Raw
        $content | Should -Match 'DaysBeforeExpiry'
