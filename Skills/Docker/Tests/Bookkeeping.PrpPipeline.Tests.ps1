...
BeforeAll {
    $script:CurrentsbkRecon = "C:\Repos\currentsbk\Skills\Bookkeeping\Scripts\reconciliation"
}

Describe "Invoke-PrpOrgPipeline.ps1 — failed-account exclusion from cross-account matching" -Tag "Bookkeeping", "Regression" {
    BeforeAll {
...
Describe "Build-TAS.ps1 — atomic TAS write" -Tag "Bookkeeping", "Regression" {
    BeforeAll {
