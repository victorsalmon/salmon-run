...
BeforeAll {
    ...
    $script:TargetPath = Join-Path $script:RepoRoot "Skills" "Docker" "1Fleet.ps1"
}

Describe "1Fleet.ps1" -Tag "Docker" {
    ...
    It "does not need [OutputType()] — entrypoint script with no pipeline output" {
    ...
    It "has param block with Mode parameter" {
        $content = Get-Content $script:TargetPath -Raw
