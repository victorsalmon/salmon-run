...
BeforeAll {
    ...
    $script:TargetPath = Join-Path $script:RepoRoot "Skills" "Docker" "deploy-lite.ps1"
}

Describe "deploy-lite.ps1" -Tag "Docker" {
    ...
    It "does not need [OutputType()] — deploy script with no pipeline output" {
    ...
    It "has param block with Containers, FleetUrl, and Execute parameters" {
        $content = Get-Content $script:TargetPath -Raw
        $content | Should -Match 'param\('
        $content | Should -Match 'Containers'
        $content | Should -Match 'FleetUrl'
    }
}
