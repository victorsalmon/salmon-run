...
BeforeAll {
    ...
    $script:TargetPath = Join-Path $script:RepoRoot "Skills" "Docker" "deploy.ps1"
}

Describe "deploy.ps1" -Tag "Docker" {
    ...
    It "does not need [OutputType()] — deploy orchestrator script with no pipeline output" {
    ...
    It "has param block with AutoReboot and DroneMode parameters" {
        $content = Get-Content $script:TargetPath -Raw
        $content | Should -Match 'param\('
        $content | Should -Match 'AutoReboot'
        $content | Should -Match 'DroneMode'
    }
}
