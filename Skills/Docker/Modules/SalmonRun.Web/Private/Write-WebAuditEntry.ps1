<#
.SYNOPSIS
Writes an audit log entry for web MCP operations.
.DESCRIPTION
...
function Write-WebAuditEntry {
    ...
    $mutexName = 'Global\InterclawWebAuditLog'
    $mutex = $null
    $mutexOwned = $false
    try {
        $dir = Split-Path $script:WebAuditLogPath -Parent
        ...
        if (-not $mutexOwned) {
            Write-Warning "Write-WebAuditEntry: mutex timeout for audit log '$($script:WebAuditLogPath)' after 5s"
            return
        }

        $prev = ''
        if (Test-Path $script:WebAuditLogPath) {
            $lastLine = Get-Content -LiteralPath $script:WebAuditLogPath -Tail 1 -ErrorAction SilentlyContinue
            if ($lastLine) {
                try {
        ...
        Add-Content -LiteralPath $script:WebAuditLogPath -Value $line -Encoding utf8
    }
    catch {
        ...
        if ($mutex) {
            if ($mutexOwned) {
                try { $mutex.ReleaseMutex() } catch { Write-Debug "Write-WebAuditEntry: mutex release failed: $_" }
            }
            $mutex.Dispose()
        }
    }
