...
function Test-FleetSidecarHealth {
    ...
    <#
    .NOTES
        Timeout chain: Up to 5 sequential Invoke-RestMethod calls, each -TimeoutSec 5.
        Worst-case chain sum: 25s. Called from Invoke-FleetHealthCheck (sequential, no cumulative timeout).
        Per-service monitor tokens: each service's monitor token is mounted at /run/secrets/fleet_<service>_monitor_token.
    #>
    $results = [System.Collections.Generic.List[object]]::new()
    Write-Verbose "`n[8] Sidecar Services"
    $AllSvcNames = $StackServices | ForEach-Object { ($_ -split "`t")[0] }

    function Get-ServiceMonitorToken {
        param([string]$ServiceSecretName)
        $token = Get-Content "/run/secrets/$ServiceSecretName" -Raw -ErrorAction SilentlyContinue | ForEach-Object { $_.Trim() }
        if ([string]::IsNullOrWhiteSpace($token)) {
            Write-Warning "Monitor token for $ServiceSecretName unavailable — skipping health check"
            return $null
        }
        return $token
    }
    ...
    if ($McpBrowserlessService) {
        try {
            $token = Get-ServiceMonitorToken -ServiceSecretName "fleet_browserless_monitor_token"
            if ($token) {
    ...
    if ($FunnelProxyService) {
        try {
            $token = Get-ServiceMonitorToken -ServiceSecretName "fleet_funnel_monitor_token"
            if ($token) {
                $authHeaders = @{ "Authorization" = "Bearer $token" }
    ...
    if ($McpOpencodeService) {
        try {
            $token = Get-ServiceMonitorToken -ServiceSecretName "fleet_monitor_token"
            if ($token) {
                $authHeaders = @{ "Authorization" = "Bearer $token" }
            } else {
                $authHeaders = @{}
