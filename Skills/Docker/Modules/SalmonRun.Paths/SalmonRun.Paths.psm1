...
<#
.SYNOPSIS
    Returns the SalmonRun runtime home.
    Honors SALMON_RUN_HOME, then falls back to ~/.salmon.
.OUTPUTS
    System.String
#>
function Get-SalmonHome {
    [CmdletBinding()]
    param()
    $home = $env:SALMON_RUN_HOME
    if ([string]::IsNullOrWhiteSpace($home)) {
        $home = Join-Path (Get-HomeDir) '.salmon'
    }
    return $home
}
...
<#
.SYNOPSIS
    Returns the canonical flat task queue root (e.g., ~/.salmon/Tasks).
.OUTPUTS
    System.String
#>
...
Export-ModuleMember -Function 'Get-SalmonRunRepoRoot','Get-HomeDir','Get-SalmonHome','Get-SalmonTaskRoot','Get-RepoRoot','Get-SkillsRoot','Resolve-SkillPath','Reset-SalmonRunPathCache' -Alias 'Get-InterclawRepoRoot','Reset-InterclawPathCache'
