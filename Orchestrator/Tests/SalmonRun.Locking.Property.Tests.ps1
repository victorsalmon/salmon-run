...
BeforeAll {
    ...
    $lockingPrivateDir = Join-Path $script:repoRoot 'Orchestrator/Modules/SalmonRun.Locking/Private'
    if (Test-Path $lockingPrivateDir) {
        Get-ChildItem -Path $lockingPrivateDir -Filter '*.ps1' | ForEach-Object { . $_.FullName }
    }
    $lockingPublicDir = Join-Path $script:repoRoot 'Orchestrator/Modules/SalmonRun.Locking/Public'
    ...
    . (Join-Path $script:repoRoot 'Orchestrator/Modules/SalmonRun.Core/Public/Convert-PidSafe.ps1')
    $lifecyclePublicDir = Join-Path $script:repoRoot 'Orchestrator/Modules/SalmonRun.AgentLifecycle/Public'
