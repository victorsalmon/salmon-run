...
function Handle-OrphanStatus {
    ...
    param([System.IO.FileInfo]$File, [string]$Agent, [Alias('ORCHESTRATORDir')][string]$RepoDir, [string]$RescueKind = "RESCUE")
...
function Rescue-OrphanedLocks {
    ...
    foreach ($file in $orphanedFiles) {
        ...
        if ($ageMins -ge $timeoutThresholdMins) {
            ...
            Handle-OrphanStatus -File $file -Agent $fileAgent -RepoDir $RepoDir -RescueKind "RESCUE_HARD_TIMEOUT"
        }
    }
    ...
    foreach ($file in $orphanedFiles) {
        ...
        if ($aliveInfo -and $aliveInfo.Stale) {
            Handle-OrphanStatus -File $file -Agent $fileAgent -RepoDir $RepoDir -RescueKind "RESCUE_STALE"
            continue
        }
        ...
        if ($pidAlive -eq $false) {
            ...
            Handle-OrphanStatus -File $file -Agent $fileAgent -RepoDir $RepoDir -RescueKind "RESCUE_PID"
            continue
        }
        ...
        Handle-OrphanStatus -File $file -Agent $fileAgent -RepoDir $RepoDir
    }
...
function Clear-AgentArtifacts {
    ...
    if (Test-Path $workingDir) {
        ...
        foreach ($f in $leftover) {
            Handle-OrphanStatus -File $f -Agent $AgentId -RepoDir $RepoDir -RescueKind "AGENT_COMPLETED"
        }
