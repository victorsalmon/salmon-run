...
function Handle-ExitCodeDefault {
    param($ProcId, $ExitCode, $PidToAgent, $RetryCounts, $MaxSubprocessRetries, $RepoDir, $Namespace = "unknown", $AgentOutcomes)
    ...
    return Invoke-CrashRecovery -ExitCode $ExitCode -PidToAgent $PidToAgent -ProcId $ProcId -RetryCounts $RetryCounts -MaxSubprocessRetries $MaxSubprocessRetries -RepoDir $RepoDir -AgentOutcomes $AgentOutcomes
}
...
function Invoke-CrashRecovery {
    ...
    if ($retryCount -le $MaxSubprocessRetries) {
        ...
        if ($workingFile) { Handle-OrphanStatus -File $workingFile -Agent $PidToAgent[$ProcId] -RepoDir $RepoDir }
    }
    return $RetryCounts
}
...
function Test-StreamAgentAlive {
    ...
    #>
    param([string]$StreamId, [string]$RepoDir)
    if ([string]::IsNullOrWhiteSpace($StreamId)) { return $false }
    $hb = Join-Path $RepoDir "Tasks/Logs/agents/$StreamId.heartbeat"
...
function Invoke-ReadFilesystemState {
    ...
    foreach ($sd in $streamDirs) {
        ...
        if ($hasComplete -or -not $hasPlanFiles) {
            ...
            $agentAlive = Test-StreamAgentAlive -StreamId $meta.Id -RepoDir $RepoDir
            if ($agentAlive) {
    ...
    foreach ($f in $staleWorkingFiles) {
        ...
        try {
            ...
            if ($alive.Stale) {
                ...
                Handle-OrphanStatus -File $f -Agent $fileAgent -RepoDir $RepoDir -RescueKind "RESCUE_STALE"
