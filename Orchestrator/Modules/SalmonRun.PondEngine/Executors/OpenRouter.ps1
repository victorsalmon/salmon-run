<#
.SYNOPSIS
    OpenRouter provider executor for the salmon-run PondEngine.

.DESCRIPTION
    Runs the `openrouter` CLI. It resolves `OPENROUTER_API_KEY`, builds
    `openrouter run --command work-{role}-once --model {model} --effort {effort}
...
function Resolve-OpenRouterCredential {
    $key = $null
    if (Get-Command Get-SalmonRunCredential -ErrorAction SilentlyContinue) {
        try {
            $key = Get-SalmonRunCredential -Name 'OPENROUTER_API_KEY'
        } catch {
            $key = $null
        }
    }
    ...
    if ([string]::IsNullOrWhiteSpace($key)) {
        throw "OpenRouter executor: OPENROUTER_API_KEY is not configured. Provide it in ~/.salmon/.env or as an environment variable."
    }
    return $key
}
...
function Invoke-OpenRouterProvider {
    ...
    $credential = Resolve-OpenRouterCredential
    [Environment]::SetEnvironmentVariable('OPENROUTER_API_KEY', $credential, 'Process')

    $outLog = Join-Path $LanePath 'openrouter.log'
    $errLog = Join-Path $LanePath 'openrouter.err'
    ...
    $exitCode = 1
    try {
        $process = Start-Process -FilePath 'openrouter' -ArgumentList $argumentList `
...
if ($MyInvocation.InvocationName -ne '.') {
    try {
        exit (Invoke-OpenRouterProvider)
    } catch {
